...
Module modMain
  ...
  Public objLVed As DgvHandle               ' Handler for LemmaVern editor dgv
  Public objMetaEd As DgvHandle             ' Handler for TEI-[metadata] information (on frmMain)
  ...
  Public strLastSyntaxUpdate As String = "" ' Last directory for updating syntax
  Public strLastConvert As String = ""      ' Last directory used for conversion
  Public strLastConvertDst As String = ""   ' Last destination 
  Public strLastFoliaSrc As String = ""     ' Last source directory for conversion to folia
  Public strLastFoliaDst As String = ""     ' Last destination directory for conversion to folia
  ...
  Public strCurrentTransLang As String = "" ' Name of currently selected translation language
  Public strCurrentEthno As String = ""     ' The language of the current psdx file we are working on
  Public strDbaseXpathFilter As String = "" ' Current filter
  Public bDepWholeText As Boolean = False   ' Show whole text or not
  Public bShowCode As Boolean               ' Setting of chbShowCode
  Public bRefEditMode As Boolean = False    ' Ref editing mode
  ...
  Public bEdtDirty As Boolean = False       ' The "Editor" tab page needs to be re-drawn
  Public bDepChg As Boolean = False         ' Dep settingi s changing...
  ...
  Public tdlRemain As DataSet = Nothing     ' Remain set
  Public tdlMeta As DataSet = Nothing       ' This contains basically the TEI-METADATA stuff
  ...
  Public ndxCurrentNode As XmlNode = Nothing ' Currently selected node (deepest)
  Public ndxCurrentTreeNode As XmlNode = Nothing  ' Currently selected node in tree editing
  ...
  Public bUpdStartup As Boolean = True      ' Check updates at startup
  ...
  Public strMorphRemainFile As String = ""              ' Full location for the remain dictionary file
  ...
  Public intNumFeaturesAllowed As Integer = NUM_FEATURES_ALLOWED
  Public strDefPeriodText As String = _
    "<?xml version='1.0' standalone='yes'?>" & vbCrLf & _
    "<PeriodList>" & vbCrLf & _
    "</PeriodList>" & vbCrLf
  Public strSoftwareMsg As String = "http://erwinkomen.ruhosting.nl/software/Cesax/msg.txt"
  ' =================================GLOBAL CONSTANTS===========================================================
  ...
  Public Const CAT_VBTYPE As String = "VbType"          ' Specific VbType value, possibly dependent upon MED, OED or BT number
  Public Const CAT_VBUNACC As String = "VbUnacc"        ' Unaccusatives through *psdx* files coded with "U-" labels
  ...
  Public Sub ReadSettings()
    ...
    Try
      ...
      strLastSyntaxUpdate = GetSettingValue("LastSyntaxUpdate") & ""
      strLastConvert = GetSettingValue("LastConvertDir") & ""
      strLastConvertDst = GetSettingValue("LastConvertDirDst") & ""
      strLastFoliaSrc = GetSettingValue("LastFoliaSrc") & ""
      strLastFoliaDst = GetSettingValue("LastFoliaDst") & ""
      ...
      ' Check for updating on startup??
      bUpdStartup = (My.Settings.UpdStartup = "True")
  ...
  Public Sub MorphDictIni(Optional ByVal bForce As Boolean = False, _
    ...
    Try
      ...
      If (tdlMorphDict Is Nothing) OrElse (bForce) Then
        ...
        End If
      End If
      If (tdlRemain Is Nothing) AndAlso (strMorphRemainFile <> "") Then
        If (Not ReadDataset(strMorphDictScheme, strMorphRemainFile, tdlRemain)) Then
          ' Tell the user
          Logging("Could not read remain dictionary: " & strMorphRemainFile)
          Status("Could not read remain dictionary: " & strMorphRemainFile)
          ' But return OK anyway!
          Exit Sub
        End If
      End If
  ...
  Public Function MakeFeatDict(ByVal strCategory As String, Optional ByVal strFile As String = "") As Boolean
    ...
    Dim arCol() As String         ' Columns per line

    Try
      ' Initialise the feature dictionary
      If (Not FeatDictInit(True)) Then Return False
      ' If this is "Adv", then we know what to do
      Select Case strCategory
        Case CAT_NP
        Case CAT_VB, CAT_VBTYPE       ' Unaccusative verbs -- take from a list
          ' Should be a file
          ...
          For intI = 0 To arLine.Length - 1
            ...
            If (arLine(intI) <> "") Then
              ...
              If (InStr(arLine(intI), ";") > 0) Then
                ' Semicolumns have preference as dividers
                arCol = Split(arLine(intI), ";")
              Else
                ' Default is the tab
                arCol = Split(arLine(intI), vbTab)
              End If
              Select Case arCol.Length
                Case 3
                  ' Add this feature with the indicated SOURCE category: [Lemma - MED/OED/BT - VbType]
                  If (Not AddFeatDict(strCategory, arCol(2), strCurrentPeriod, arCol(0), arCol(1))) Then Return False
                Case 2
                  ' Add this feature with the indicated category: [Lemma - VbType]
                  If (Not AddFeatDict(strCategory, arCol(1), strCurrentPeriod, arCol(0))) Then Return False
                Case 1
                  ' Add this feature
  ...
  ' ------------------------------------------------------------------------------------
  Public Function GetFeatEntry(ByVal strCategory As String, ByVal strWord As String, Optional ByVal strSrcNum As String = "") As String
    Dim strOut As String = ""   ' The resulting features
    Dim dtrFound() As DataRow   ' Result of SELECT function
      ...
      Select Case strCategory
        ...
        Case CAT_VBTYPE
          ...
          If (strSrcNum = "") Then
            dtrFound = tblFeatDict.Select("Vern = '" & strWord.Replace("'", "''") & "'")
          Else
            dtrFound = tblFeatDict.Select("Vern = '" & strWord.Replace("'", "''") & "' AND Seek='" & strSrcNum & "'")
          End If
          ' Any result?
          If (dtrFound.Length > 0) Then
            ' Get the type, which can be anything
            strOut = dtrFound(0).Item("Type").ToString
          End If
        Case CAT_PNORM
          ' Try find this entry
          dtrFound = tblFeatDict.Select("Vern = '" & strWord.Replace("'", "''") & "'")
  ...
  Public Function AddFeatDict(ByVal strCategory As String, ByVal strType As String, ByVal strPeriod As String, _
                               ByVal strVern As String, Optional ByVal strSrcNum As String = "") As Boolean
    Dim strBare As String = ""    ' The bare vernacular item
    Dim strSeek As String = ""    ' The vernacular item to be used for seeking

    Try
      ' Initialise the feature dictionary
      If (Not FeatDictInit(False)) Then Return False
      ' If this is "Adv", then we know what to do
      Select Case strCategory
        Case CAT_NP
        Case CAT_VB
        ...
        Case CAT_VBTYPE
          ...
          If (strSrcNum = "") Then
            tblFeatDict.Rows.Add(strVern, strVern, strVern, strPeriod, strCategory, strType)
          Else
            tblFeatDict.Rows.Add(strVern, strVern, strSrcNum, strPeriod, strCategory, strType)
          End If
  ...
  '---------------------------------------------------------------------------------------------------------
  Public Sub Logging(ByVal strIn As String, Optional ByVal bDoStatus As Boolean = True)
    ...
    Try
      ...
      End With
      If (bDoStatus) Then
        ' Show this in the status too
        Status(strIn)
      End If
    Catch ex As Exception
      ' Show error
      HandleErr("modMain/Logging error: " & ex.Message & vbCrLf & ex.StackTrace & vbCrLf)
    End Try
  End Sub
  '---------------------------------------------------------------------------------------------------------
  ' Name:       LoadOneFile()
  ...
  Public Function LoadOneFile(ByVal strFile As String) As Boolean
    ...
    Try
      ...
      If (Not SetRecent(strFile)) Then Return False
  ...
  '---------------------------------------------------------------------------------------------------------
  ' Name:       SetRecent()
  ' Goal:       Set [strFile] as the most recent one
  ' History:
  ' 05-04-2014  ERK Created
  '---------------------------------------------------------------------------------------------------------
  Public Function SetRecent(ByVal strFile As String) As Boolean
    Dim intI As Integer       ' Counter
    Dim intPrev As Integer    ' Number where recent file with same name is already kept
    Try
      ' Check if this file already exists in the Recent ones
    ...
    Catch ex As Exception
      ' Show error
      HandleErr("modMain/SetRecent error: " & ex.Message & vbCrLf & ex.StackTrace & vbCrLf)
      ' return failure
      Return False
    End Try
  End Function
  ...
  Public Function InitCurrentFile() As Boolean
    Try
      ...
      pdxList = pdxCurrentFile.SelectNodes("./descendant::forest")
      ' Anything back?
  ...
  Public Function ShowTranslation(ByVal bShowHtml As Boolean) As Boolean
    ...
    Try
      ' Validate
      If (pdxList.Count = 0) Then Return False
      ' Get the line number
      intLine = GetEdtLine()
  ...
  Public Function MakeChart(ByVal bDoSub As Boolean, ByVal bAllSc As Boolean, ByVal strOutType As String, _
    ...
    Try
      ...
      While (Not ndxThis Is Nothing) AndAlso (intI <= intCount)
        ...
        strSeg = GetSeg(ndxThis, strCurrentTransLang)
  ...
  '---------------------------------------------------------------------------------------------------------
  Public Function AddTransLang(ByVal strLang As String, ByRef bChanges As Boolean) As Boolean
    Dim ndxThis As XmlNode = Nothing  ' <forest> node
    Dim ndxSeg As XmlNode             ' Segment
    Dim intI As Integer               ' Counter
    Dim intPtc As Integer             ' Percentage
    Dim intCount As Integer           ' Number of <forest> lines
    Dim intChg As Integer = 0         ' Number of changes

    Try
      ' Validate
      If (pdxCurrentFile Is Nothing) Then Logging("AddTransLang: no file loaded") : Return False
      ' Find first forest element - depending on the project type
      If (Not GetFirstForest(pdxCurrentFile, ndxThis)) Then Logging("AddTransLang error: cannot find first line") : Exit Function
      ' Get number of nodes
      ...
      While (Not ndxThis Is Nothing)
        ' Show where we are
        intPtc = intI * 100 \ intCount
        Status("Adding language " & intPtc & "%", intPtc)
        ' Check if segment already exists
        ndxSeg = ndxThis.SelectSingleNode("./div[@lang='" & strLang & "']/seg")
        If (ndxSeg Is Nothing) Then
          ' Segment does not yet exist, so create it
          ndxSeg = AddXmlChild(ndxThis, "div", "lang", strLang, "attribute", "seg", "", "child")
          intChg += 1
        End If
        ' GO to next forest
        ndxThis = ndxThis.SelectSingleNode("./following-sibling::forest") : intI += 1
      End While
      ' Note changes
      If (intChg > 0) Then bChanges = True
      ' Show success
      Return True
    Catch ex As Exception
      ' Show error
      HandleErr("modMain/AddTransLang error: " & ex.Message & vbCrLf & ex.StackTrace & vbCrLf)
      ' Return failure
      Return False
    End Try
  End Function
  ...
  Public Sub ViewFileContent(ByVal strLang As String, Optional ByVal strGloss As String = "")
    ...
    Try
      ...
      ' Ask the format
      If (strGloss = "") OrElse (MsgBox("View gloss in column?", MsgBoxStyle.YesNo) = MsgBoxResult.No) Then
        ' Step through them
        intI = 1
        While (Not ndxThis Is Nothing)
          ' Show where we are
      ...
      Else
        ' Start table
        colBack.Add("<table><tr><td>#</td><td><b>Org</b></td><td><b>" & strGloss & "</b></td></tr>")
        ' Step through them
        intI = 1
        While (Not ndxThis Is Nothing)
          ' Show where we are
          ...
          strGls = GetSeg(ndxThis, strGloss)
          ' Add line and the gloss to collection
          colBack.Add("<tr><td align='top'>" & ndxThis.Attributes("forestId").Value & _
                      "</td><td align='top'>" & strSeg & "</td><td align='top'>" & strGls & "</td></tr>")
          ' GO to next forest
          ndxThis = ndxThis.NextSibling : intI += 1
        End While
        ' Finish table
  ...
  Public Function GotoSection(ByVal intSection As Integer) As Boolean
    Try
      ...
      Else
        ...
        ' Determine the number of <forest> elements in this section
        intSectSize = intSectLast - intSectFirst
      End If
      ' ============= DEbugging ===========
      'Debug.Print("First forest = " & pdxList(intSectFirst).Attributes("forestId").Value)
      'Debug.Print("Last  forest = " & pdxList(intSectLast).Attributes("forestId").Value)
      ' ===================================
      ' Return success
      Return True
    Catch ex As Exception
    ...
    Try
      ...
      For intI = 0 To intSectSize - 1
        ...
        If (ndxForest.Attributes("Section") Is Nothing) Then
          ...
          If (ndxForest.Attributes("Para") IsNot Nothing) Then
            ' Signal that this is the start of a paragraph
            rtbThis.SelectionIndent = 0
          Else
            rtbThis.SelectionIndent = 0
          End If
        Else
  ...
  Public Function GetPde(ByVal intLine As Integer) As String
    ...
    Try
      ...
      ' Get the translation
      Return ndxForest.SelectSingleNode("./div[@lang='" & strCurrentTransLang & "']/seg").InnerText
    Catch ex As Exception
      ' Show error
  ...
  Public Sub SetPde(ByVal intLine As Integer, ByVal strPde As String)
    ...
    Try
      ...
      ndxForest = pdxList(intLine + intSectFirst)
      ' Set the translation
      ndxForest.SelectSingleNode("./div[@lang='" & strCurrentTransLang & "']/seg").InnerText = strPde
    Catch ex As Exception
      ' Show error
  ...
  Public Function DoTransRead(ByRef pdxThis As XmlDocument) As Boolean
    ...
    Try
      ...
      pdxSrc = pdxThis.SelectNodes("//seg[parent::div[@lang='" & strCurrentTransLang & "']]")
      pdxDst = pdxCurrentFile.SelectNodes("//seg[parent::div[@lang='" & strCurrentTransLang & "']]")
      ' Check their numbers
  ...
  Public Function GetFirstTrans() As Integer
    ...
    Try
      ...
      For intI = 0 To pdxList.Count - 1
        ...
        If (ndxThis.SelectSingleNode("./div[@lang='" & strCurrentTransLang & "']").InnerText = "") Then
  ...
  Public Function GetTextTrans(ByVal intLine As Integer, ByVal bSection As Boolean, _
    ...
    Try
      ...
      For intI = intStart To intEnd
        ...
        strBare = ndxThis.SelectSingleNode("./div[@lang='" & strCurrentTransLang & "']").InnerText
        '' ========== ListView =====
  ...
  Public Function GetTransHtml(ByVal bSection As Boolean, Optional ByVal intLine As Integer = -1) As String
    ...
    Try
      ...
      For intI = intStart To intEnd
        ...
        ' Get the Original and the English
        strOrg = ndxThis.SelectSingleNode("./div[@lang='org']").InnerText
        strTrn = ndxThis.SelectSingleNode("./div[@lang='" & strCurrentTransLang & "']").InnerText
        ' Put this in HTML
  ...
  Public Function MyTrim(ByVal strIn As String) As String
    Dim strSkip As String = vbCrLf & vbTab & " " & ChrW(&HA0)  ' What to skip
  ...
  Public Function TrySaveResDb(ByVal strFile As String) As Boolean
    Try
      ...
      ' Show we saved it
      Status("Saved results database " & strFile & " at: " & Format(Now, "g"))
      ' Return success
      Return True
    Catch ex As Exception
      ' Show error
  ...
  ' ---------------------------------------------------------------------------------------------------------
  Public Function GetName(ByVal strQuestion As String, Optional ByVal strDefault As String = "") As String
  ...
  Public Function XmlFileType(ByVal strFile As String) As String
    ...
    Try
      ...
      If (Not IO.File.Exists(strFile)) Then Logging("File not found: " & strFile) : Return ""
  ...
  ' ------------------------------------------------------------------------------------------------------------
  ' Name:   TransLangCheck
  ' Goal:   Check presence of at least [org] and [eng] translation languages
  ' History:
  ' 17-06-2014  ERK Created
  ' ------------------------------------------------------------------------------------------------------------
  Public Function TransLangCheck(ByRef bChanged As Boolean) As Boolean
    Dim ndxThis As XmlNode = Nothing  ' Working node
    Dim ndxList As XmlNodeList        ' List of nodes
    Dim strLng As String = ""         ' One language
    Dim arNeed() As String = {"org", "eng"}
    Dim intI As Integer               ' Counter
    Dim intJ As Integer               ' Counter
    Dim intPtc As Integer             ' Percentage
    ...
    Try
      ' Validate
      If (pdxCurrentFile Is Nothing) Then Status("TransLangCheck: first load a text") : Return False
      ' Find out which back translations there are from the first <forest>
      If (Not GetFirstForest(pdxCurrentFile, ndxThis)) Then Status("TransLangCheck: could not find <forest>") : Return False
      ' Check presence of obligatory translation lines
      For intI = 0 To arNeed.Length - 1
        ' Get the language
        strLng = arNeed(intI)
        ' Check presence of [org]
        ndxList = ndxThis.SelectNodes("./child::div[@lang = '" & strLng & "']")
        If (ndxList.Count = 0) Then
          ' The [org] needs to be added
          If (Not AddTransLang(strLng, bChanged)) Then Status("TransLangCheck: could not add language @" & strLng) : Return False
        End If
        If (strLng = "org") Then
          ' it needs to be filled...
          ' Visit all the <forest> lines
          For intJ = 0 To pdxList.Count - 1
            ' Where are we?
            intPtc = (intI + 1) * 100 \ pdxList.Count
            Status("Re-analyzing " & intPtc & "%", intPtc)
            ' Re-analyze this line
            If (eTreeSentence(pdxList(intI), ndxThis)) Then bChanged = True
          Next intJ
        End If
      Next intI
      ' Return positively
    ...
    Catch ex As Exception
      ' Show error
      HandleErr("modMain/TransLangCheck error: " & ex.Message & vbCrLf & ex.StackTrace & vbCrLf)
      ' Return false
      Return False
    End Try
  End Function
  ' ------------------------------------------------------------------------------------------------------------
  ' Name:   TransLangInit
  ' Goal:   Fill the combobox with the (back) translation to be shown and to be available for editing
  ' History:
  ' 17-03-2014  ERK Created
  ' ------------------------------------------------------------------------------------------------------------
  Public Function TransLangInit(ByRef cboThis As ComboBox) As Boolean
    Dim ndxThis As XmlNode = Nothing  ' Working node
    Dim ndxList As XmlNodeList        ' List of nodes
    Dim intI As Integer               ' Counter
    Dim intEng As Integer             ' Index of "eng"

    Try
      ' Validate
      If (pdxCurrentFile Is Nothing) Then Status("TransLangInit: first load a text") : Return False
      ' Find out which back translations there are from the first <forest>
      If (Not GetFirstForest(pdxCurrentFile, ndxThis)) Then Status("TransLangInit: could not find <forest>") : Return False
      ' Get all the possible language elements
      ndxList = ndxThis.SelectNodes("./child::div[@lang != 'org']")
      ' Fill the array of BT languages
      With cboThis
        .Items.Clear() : intEng = 0
        For intI = 0 To ndxList.Count - 1
          ' Note the back translation language
          .Items.Add(ndxList(intI).Attributes("lang").Value)
          ' English back translations get the preference
          If (ndxList(intI).Attributes("lang").Value = "eng") Then intEng = intI
        Next intI
        ' Any items?
        If (.Items.Count > 0) Then
          ' Select the first one
          .SelectedIndex = intEng
        End If
      End With
      ' Return success
      Return True
    Catch ex As Exception
      ' Show error
      HandleErr("modMain/TransLangInit error: " & ex.Message & vbCrLf & ex.StackTrace & vbCrLf)
      ' Return false
      Return False
    End Try
  End Function
